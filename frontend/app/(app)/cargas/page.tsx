...
export default function CargasPage() {
  ...
  return (
    <div className="flex flex-col gap-4">
              ...
              <tr key={c.id} className="border-b border-slate-50 dark:border-borderSoft/50">
                ...
                <td className="px-3 py-3">
                  <div className="flex gap-1.5">
                    <button
                      onClick={() => setCargaParaOrdem(c)}
                      className="flex items-center gap-1 text-xs font-medium text-amber border border-amber/30 px-2 py-1 rounded-md"
                    >
                      <FileText size={11} /> Ordem
                    </button>
                    <button
      ...
      {cargaParaOrdem && (
        <EmitirOrdemModal
          carga={cargaParaOrdem}
          onClose={() => setCargaParaOrdem(null)}
        />
      )}
    </div>
  );
...
function EmitirOrdemModal({ carga, onClose }: { carga: Carga; onClose: () => void }) {
  const atribuidos: any[] = (carga as any).motoristasAtribuidos || [];
  const [motoristas, setMotoristas] = useState<Motorista[]>([]);
  const [motoristaId, setMotoristaId] = useState(atribuidos[0]?.motoristaId || "");
  const [peso, setPeso] = useState("");
  const [freteMotorista, setFreteMotorista] = useState(carga.valorMotorista ? String(carga.valorMotorista) : "");
  const [observacao, setObservacao] = useState(carga.observacoes || "");
  const [salvando, setSalvando] = useState(false);
  const [erro, setErro] = useState<string | null>(null);
  const [ordemGerada, setOrdemGerada] = useState<Ordem | null>(null);

  useEffect(() => {
    if (atribuidos.length > 0) {
      setMotoristas(atribuidos.map((a) => a.motorista));
    } else {
  ...
  async function emitir(e: React.FormEvent) {
    e.preventDefault();
    if (!motoristaId) return setErro("Selecione um motorista.");
    setSalvando(true);
    setErro(null);
    try {
      const ordem = await apiFetch("/ordens", {
        method: "POST",
        body: JSON.stringify({
          cargaId: carga.id,
          motoristaId,
          peso: peso || undefined,
          freteMotorista: freteMotorista ? Number(freteMotorista) : undefined,
          observacao: observacao || undefined,
        }),
      });
      setOrdemGerada(ordem);
    } catch (err: any) {
      setErro(err.message || "Não foi possível emitir a ordem.");
    } finally {
      setSalvando(false);
    }
  }
      ...
      <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 px-4 py-8 overflow-y-auto">
        <div className="bg-white dark:bg-surface rounded-2xl p-5 max-w-3xl w-full">
          <div className="flex items-center justify-between mb-4 print:hidden">
            <h2 className="font-display font-semibold text-slate-900 dark:text-white">Ordem emitida com sucesso</h2>
            <div className="flex gap-2">
              ...
              <button onClick={onClose} className="text-sm text-slate-500 px-3 py-2">Fechar</button>
            </div>
          </div>
          <OrdemDocumento ordem={ordemGerada} />
        </div>
      </div>
    );
  }
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 px-4 py-8 overflow-y-auto">
      <div className="bg-white dark:bg-surface rounded-2xl p-5 w-full max-w-md my-auto">
        <div className="flex items-center justify-between mb-4">
          <h2 className="font-display font-semibold flex items-center gap-2"><FileText size={16} /> Emitir ordem</h2>
          <button onClick={onClose}><X size={18} className="text-slate-400" /></button>
        </div>

        {erro && <div className="text-sm text-danger bg-danger/10 border border-danger/30 rounded-lg px-3 py-2 mb-3">{erro}</div>}

        <form onSubmit={emitir} className="flex flex-col gap-3">
          <div className="flex flex-col gap-1">
            <label className="text-xs font-medium text-slate-500">Motorista</label>
            <select
              required
              value={motoristaId}
          ...
          <div className="flex flex-col gap-1">
            ...
            <input value={peso} onChange={(e) => setPeso(e.target.value)} placeholder="Ex: 37" className="border border-slate-200 dark:border-border bg-slate-50 dark:bg-surfaceRaised rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-amber/50" />
          </div>
          <div className="flex flex-col gap-1">
            <label className="text-xs font-medium text-slate-500">Frete motorista (R$)</label>
            <input type="number" value={freteMotorista} onChange={(e) => setFreteMotorista(e.target.value)} className="border border-slate-200 dark:border-border bg-slate-50 dark:bg-surfaceRaised rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-amber/50" />
          </div>
          <div className="flex flex-col gap-1">
            <label className="text-xs font-medium text-slate-500">Observação (opcional)</label>
            <textarea value={observacao} onChange={(e) => setObservacao(e.target.value)} rows={2} className="border border-slate-200 dark:border-border bg-slate-50 dark:bg-surfaceRaised rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-amber/50" />
          </div>
          <button type="submit" disabled={salvando} className="mt-1 bg-amber text-ink font-semibold rounded-lg py-2.5 text-sm disabled:opacity-60">
            {salvando ? "Emitindo..." : "Salvar ordem e gerar PDF"}
          </button>
        </form>
      </div>
    </div>
